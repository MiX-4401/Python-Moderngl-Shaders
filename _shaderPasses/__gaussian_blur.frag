...
void main(){
    ...
    colour.rgb *= weights[0];

    if (horizontal){
        for (int i=1; i<5; i++){
            colour.rgb += texture(uTexture, uvs + vec2(offset.x * i, 0.0)).rgb * weights[i];
            colour.rgb += texture(uTexture, uvs - vec2(offset.x * i, 0.0)).rgb * weights[i];
        }
    } else {
        for (int i=1; i<5; i++){
            colour.rgb += texture(uTexture, uvs + vec2(0.0, offset.y * i)).rgb * weights[i];
            colour.rgb += texture(uTexture, uvs - vec2(0.0, offset.y * i)).rgb * weights[i];
        }
    }
